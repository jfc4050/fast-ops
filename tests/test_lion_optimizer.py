...
def test_lion(weight_decay: float, dtype: torch.dtype) -> None:
    ...
    master_params = [
        torch.rand(256, requires_grad=True, dtype=dtype, device="cuda") for _ in range(3)
    ]
    ...
    for iter in range(5):  # simulate a few training steps
        ...
        for param, param_ref in zip(params, params_ref):
            assert torch.allclose(param, param_ref, rtol=1e-4, atol=1e-4), f"param mismatch on iter {iter}"
            exp_avg = lion.state[param]["exp_avg"]
            exp_avg_ref = lion_ref.state[param_ref]["exp_avg"]
            assert torch.allclose(exp_avg, exp_avg_ref, rtol=1e-4, atol=1e-4), f"exp_avg mismatch on iter {iter}"
